...
class Toolbar {
  ...
  initialize(options) {
      ...
      if (supportsLocale) {
        try {
          ...
          for (const dh in this.context.dhs) {
            const data = this.context.dhs[dh].getGridData();
            ...
            .then((context) => {
              for (const dh in context.dhs) {
                if (typeof _dh_data_cache[dh] !== 'undefined') {
                  context.dhs[dh].loadData(_dh_data_cache[dh]);
                }
              }
    ...
    this.ready = this.loadSelectedTemplate();
  }
  ...
  createNewFile(e) {
    const dh = this.context.getCurrentDataHarmonizer();
    const isNotEmpty = dh.countRows() - dh.countEmptyRows();
          ...
          .then((context) => {
            if (!jsonData.Container) {
              ...
              for (const dh in context.dhs) {
                ...
                if (list_data) context.dhs[dh].loadData(list_data);
                else
                  alert(
  ...
  async exportFile() {
    ...
    const exportRunner = format.exportAsJson ? exportJsonFile : exportFile;
    const exportArgs = format.exportAsJson
      ? [outputMatrix, baseName]
      : [outputMatrix, baseName, format.fileType];

    await this.context.runBehindLoadingScreen(exportRunner, exportArgs);
  ...
  async loadSelectedTemplate(file = null) {
    ...
    const reloadPromise = this.context
