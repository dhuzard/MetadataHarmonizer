...
import { DEFAULT_GRID_ENGINE } from './utils/gridEngine';
...
import { createGridAdapter } from './grid/createGridAdapter';
...
class DataHarmonizer {
  ...
  requestedGridEngine = DEFAULT_GRID_ENGINE;
  ...
  constructor(root, context, options = {}) {
    ...
    this.gridEngine = options.gridEngine || context?.appConfig?.gridEngine;
    this.requestedGridEngine =
      options.requestedGridEngine ||
      context?.appConfig?.requestedGridEngine ||
      this.gridEngine;
    this.hot_override_settings = options.hot_override_settings || {};
    this.root.dataset.gridEngine = this.gridEngine;
    this.root.dataset.gridEngineRequested = this.requestedGridEngine;
  ...
  render() {
    if (this.gridAdapter?.render) {
      this.gridAdapter.render();
      return;
    }
  ...
  getGridEngineState() {
    return {
      requested: this.requestedGridEngine,
      active: this.gridEngine,
    };
  }

  loadData(data) {
    if (this.gridAdapter?.loadData) {
      this.gridAdapter.loadData(data);
      return;
    }
  ...
  getGridData(...args) {
    if (this.gridAdapter?.getData) {
      return this.gridAdapter.getData(...args);
    }

    return this.hot.getData(...args);
  }

  getCellValue(row, col) {
    if (this.gridAdapter?.getDataAtCell) {
      return this.gridAdapter.getDataAtCell(row, col);
    }
  ...
  getGridSelection() {
    if (this.gridAdapter?.getSelected) {
      return this.gridAdapter.getSelected();
    }
  ...
  getSelectedRow() {
    return this.getGridSelection()?.[0]?.[0] ?? null;
  }

  countRows() {
    if (this.gridAdapter?.countRows) {
      return this.gridAdapter.countRows();
    }
  ...
  countEmptyRows(ending = undefined) {
    if (this.gridAdapter?.countEmptyRows) {
      if (typeof ending === 'undefined') {
        return this.gridAdapter.countEmptyRows();
      }

      return this.gridAdapter.countEmptyRows(ending);
    }

    if (typeof ending === 'undefined') {
      return this.hot.countEmptyRows();
    }

    return this.hot.countEmptyRows(ending);
  }

  countCols() {
    if (this.gridAdapter?.countCols) {
      return this.gridAdapter.countCols();
    }
  ...
  countSourceRows() {
    if (this.gridAdapter?.countSourceRows) {
      return this.gridAdapter.countSourceRows();
    }

    return this.hot.countSourceRows();
  }

  removeRows(rows) {
    if (this.gridAdapter?.alter) {
      this.gridAdapter.alter('remove_row', rows);
      return;
    }

    this.hot.alter('remove_row', rows);
  }

  setCellValue(row, col, value, source = 'edit') {
    if (this.gridAdapter?.setDataAtCell) {
      this.gridAdapter.setDataAtCell(row, col, value, source);
      return;
    }

    this.hot.setDataAtCell(row, col, value, source);
  }

  replaceHiddenRows(rows) {
    const hiddenRowsPlugin = this.hot.getPlugin('hiddenRows');
    const oldHiddenRows = hiddenRowsPlugin.getHiddenRows();

    if (oldHiddenRows.length > 0) {
  ...
  async validate() {
    ...
    if (this.gridAdapter?.renderValidationFeedback) {
      this.gridAdapter.renderValidationFeedback(this.invalid_cells);
    }
  }
  ...
  createHot(data = []) {
    ...
    this.gridAdapter = createGridAdapter(this.gridEngine, this);

    if (this.gridAdapter?.isRevoGrid?.()) {
      this.createRevoGrid(data);
      return;
    }

    this.hot = this.gridAdapter.initialize(this.hotRoot);
  ...
  createRevoGrid(data = []) {
    ...
    this.gridAdapter.initialize(this.hotRoot, {
      data,
      sections: this.sections,
      slots: this.slots,
      hotOverrideSettings: this.hot_override_settings,
    });
    ...
    this.hot = this.gridAdapter.getEngine();
  }
  ...
  updateSources(slot) {
    ...
    slot.sources.forEach((source) => {
      ...
      if (!(source in slot.permissible_values)) {
        alert(
          `Schema Programming Error: Slot range references enumeration ${source} but this was not found in enumeration dictionary`
        );
      }
      // This case catches empty menus, which merits an error in end-user DH but is ok in Schema Editor.
      else if (
        this.schema.name !== 'DH_LinkML' &&
        !slot.permissible_values[source]
      ) {
        alert(
          `Schema Programming Error: Slot range enumeration's ${source} has no selections!`
        );
      } else
