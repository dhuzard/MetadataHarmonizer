...
import { getGridEngineInScope, resolveGridEngine } from './utils/gridEngine';
...
class AppConfig {
  constructor(template_path = null, options = {}) {
    const effectiveTemplatePath = template_path || getTemplatePathInScope();
    const resolvedGridEngine = resolveGridEngine(
      options.gridEngine ?? getGridEngineInScope(),
      {
    ...
    this.template_path = effectiveTemplatePath;
    this.requestedGridEngine = resolvedGridEngine.requested;
    this.gridEngine = resolvedGridEngine.active;
    this.gridEngineImplemented = resolvedGridEngine.implemented;
  }
}
export default class AppContext {
  ...
  applyGridEngineMetadata() {
    const rootElement = document.documentElement;
    rootElement.dataset.gridEngineRequested =
      this.appConfig.requestedGridEngine;
    rootElement.dataset.gridEngineActive = this.appConfig.gridEngine;
    rootElement.dataset.gridEngineImplemented = String(
      this.appConfig.gridEngineImplemented
    );
  }
  ...
  async reload(template_path, locale = null, forced_schema = null) {
    ...
    this.appConfig = new AppConfig(template_path, {
      gridEngine: this.appConfig?.requestedGridEngine,
    });
    this.applyGridEngineMetadata();
      ...
      .forEach((obj, index) => {
        if (obj.length > 0) {
          ...
          const handsOnDH = new DataHarmonizer(dhSubroot, this, {
            ...
            gridEngine: this.appConfig.gridEngine,
            requestedGridEngine: this.appConfig.requestedGridEngine,
  ...
  */
        ...
        Object.entries(class_obj.attributes ?? {}).forEach(
          ([slot_name, attribute]) => {
            if (attribute.annotations?.foreign_key?.value) {
              ...
              if (!relations[class_name].parent) {
                relations[class_name].parent = {};
              }
              if (!relations[class_name].parent[foreign_class]) {
                relations[class_name].parent[foreign_class] = {};
              }
              relations[class_name].parent[foreign_class][slot_name] =
                foreign_slot_name;

              // And reverse relation
              if (!relations[foreign_class].child) {
                relations[foreign_class].child = {};
              }
              if (!relations[foreign_class].child[class_name]) {
                relations[foreign_class].child[class_name] = {};
              }
              relations[foreign_class].child[class_name][foreign_slot_name] =
                slot_name;

              // And dependent slots via foreign key relations.
              if (!relations[class_name].dependent_slots) {
                relations[class_name].dependent_slots = {};
              }
        ...
        Object.entries(class_obj.unique_keys ?? {}).forEach(
          ([key_name, key_obj]) => {
            Object.entries(key_obj.unique_key_slots ?? {}).forEach(
              ([index, slot_name]) => {
                if (!relations[class_name].unique_key_slots) {
                  relations[class_name].unique_key_slots = {};
                }
                if (!relations[class_name].unique_key_slots[slot_name]) {
  ...
  crudGetDependents(class_name, ptr = -1, stack = new Map()) {
    ...
    for (const dependent_name in children) {
      if (!stack.has(dependent_name)) {
        stack.set(dependent_name, true);
        this.crudGetDependents(dependent_name, ptr + 1, stack);
      }
    }
    return stack;
  }
  ...
  crudFilterByForeignKey(class_name) {
    ...
    let rowsToHide = arrayRange(0, dh.countSourceRows());
    ...
    if (errors.length == 0) {
      rowsToHide = rowsToHide.filter((row) => {
        for (const [slot_name, value] of Object.entries(required_selections)) {
          ...
          const cellValue = dh.getCellValue(row, col);
    ...
    dh.replaceHiddenRows(rowsToHide);
  }
  ...
  crudGetSelectedTableSlotValue(class_name, slot_name) {
    ...
    const row = dh.getSelectedRow();
    if (row !== null) {
      const col = dh.getColumnIndexByFieldName(slot_name);
      const value = dh.getCellValue(row, col);
  ...
  crudGetDependentChanges(class_name, do_action = false) {
    ...
    for (const [dependent_name, obj] of this.relations[class_name].dependents) {
      ...
      Object.entries(parents).forEach(([parent_name, parent]) => {
        ...
        const parent_row = pdh.getSelectedRow();
  ...
  crudGetKeyVals(class_name, dependent_name, row) {
    ...
    Object.entries(linked_slots).forEach(([class_slot, child_slot]) => {
      ...
      let value = pdh.getCellValue(row, col);
  ...
  crudFindByKeyVals(dh, keyVals, row = 0) {
    const total_rows = dh.countSourceRows(); // .countSourceRows();
    while (row < total_rows) {
      // .every() causes return on first break
      if (
        Object.entries(keyVals).every(([slot_name, value]) => {
          const col = dh.getColumnIndexByFieldName(slot_name);
          return value == dh.getCellValue(row, col);
        })
      )
        break;
      else row++;
  ...
  crudHasCompleteUniqueKey(
    ...
    let complete = Object.entries(key_obj.unique_key_slots)
      ...
      .every(([index, slot_name]) => {
        ...
        // Not a changed slot.
        let col = dh.getColumnIndexByFieldName(slot_name);
        let value = dh.getCellValue(row, col);
