  ...
  prepare(row, col, prop, td, value, cellProperties) {
    ...
    function filter(e, controller) {
      ...
      if (!controller.oldTD || controller.oldTD !== controller.TD) {
        ...
        controller.choices =
          controller.TEXTAREA.nextElementSibling.querySelectorAll('td');
      }
    ...
    Object.assign(this.htOptions, {
      ...
      cells: function (row, col) {
        ...
        cellProp.className =
          'selectDepth_' + (cellProperties.source[row]?.depth || '0');
        return cellProp;
      },
    });
    /*
...
export const keyValueListRenderer = function (
  ...
  }`; // This directly sets what is displayed in the cell
};
  ...
  return function (hot, TD, row, col, prop, value, cellProperties) {
    ...
    if (!isEmptyUnitVal(value)) {
      label = value
        ...
        .map((value_item) => {
          const choice = cellProperties.source.find(
            ({ _id }) => _id === value_item
          );
          if (!choice)
            console.warn(
              `${value_item} not in permissible_values for ${field.name} slot.`
            );
